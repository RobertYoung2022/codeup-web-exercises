...
(function () {
    ...
    let emailString = users.map(user => user.email);
    console.log(emailString);

    let totalYears = users.reduce((previousValue, currentValue) => {
        return previousValue + currentValue.yearsOfExperience;
    }, 0);
    console.log(totalYears);

    // getting the average of years of experiences (BONUS)
    let avgExp = totalYears / users.length;
    console.log(avgExp);


    // let longestEmail = emailString.reduce((previousValue, currentValue) => {
